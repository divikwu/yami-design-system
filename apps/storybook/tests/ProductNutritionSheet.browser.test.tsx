...
test("Mobile nutrition reuses the table, traps focus, scrolls independently and restores the PC disclosure", async () => {
  ...
  try {
    ...
    for (const width of [360, 375, 402]) {
      ...
      expect(sheet.getBoundingClientRect().right - closeRect.right).toBe(16);
      expect(closeRect.top - sheet.getBoundingClientRect().top).toBe(12);
      expect(close.querySelector("img")!.getBoundingClientRect().width).toBe(24);
      expect(close.querySelector("img")!.getBoundingClientRect().height).toBe(24);
