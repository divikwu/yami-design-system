...
test.each(['text', 'images'] as const)('%s keeps a diagonal route into the panel open', async presentation => {
  ...
  try {
    ...
    const neighbor = container.querySelector<HTMLAnchorElement>('[data-slot="header-categories"] a')!;
...
test.each(['text', 'images'] as const)('%s Categories opens on mouse hover without stealing focus', async presentation => {
  ...
  try {
    ...
    expect(menu()!.querySelector('[data-level="1"]')!.textContent).toContain('Skincare');
    await trigger.click();
    expect(menu()).not.toBeNull();
...
test.each([1024, 1280, 1920])("navigates PC categories and preserves mobile behavior at %ipx", async (width) => {
  ...
  try {
    ...
    await trigger.click();
    await expect.poll(() => getMenu()?.dataset.columns).toBe("2");
    ...
    // Changing a parent must discard the previous third-level panel.
    await trigger.click();
...
test("supports localized categories, viewport height, tab dismissal and sticky headers", async () => {
  ...
  try {
    ...
    await expect.poll(() => menu.getBoundingClientRect().top).toBeCloseTo(header.getBoundingClientRect().bottom - 8, 0);
