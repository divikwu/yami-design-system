...
test("keeps Quick Start above five visible groups and three direct Storybook links", async ({ page }) => {
  ...
  for (const name of ["使用组件库", "用 AI 创建", "多人协作", "扩展与维护", "资源与帮助"]) {
...
for (const locale of ["zh", "en"] as const) {
  ...
  for (const width of [375, 733, 1440]) {
    test(`${locale} guide groups page creation under AI at ${width}px`, async ({ page }) => {
      ...
      await expect(ai.getByRole("link").nth(1)).toHaveText(labels.component);
      await expect(ai.getByRole("link").nth(2)).toHaveText(labels.title);
      await expect(ai.getByRole("link").nth(3)).toHaveText(labels.check);
  ...
  test(`${locale} pagination follows setup, creation, then page checks`, async ({ page }) => {
    ...
    for (const [slug, title] of [["prepare-environment", labels.prepare], ["create-components", labels.component], ["choose-starting-point", labels.title], ["review-checklist", labels.check]]) {
  ...
  test(`${locale} routes prompts from the start page into component and page workflows`, async ({ page }) => {
    ...
    await expect(componentPrompts).toHaveCount(2);
    for (const prompt of await componentPrompts.all()) {
...
test("mobile creation navigation stays accessible with the compact guide", async ({ page }) => {
  ...
  await expect(dialog.getByRole("navigation", { name: "YAMI 文档", exact: true }).getByRole("link")).toHaveCount(20);
