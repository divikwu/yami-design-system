...
test("chapter menu uses an anchored list with a neutral selected state", async ({ page }) => {
  ...
  await expect(list.getByRole("option", { selected: true })).toHaveText("选择你的路径");
  await expect(list.getByRole("option").first()).toHaveCSS("font-size", "14px");
  await list.getByRole("option", { name: "先复用，再调整", exact: true }).click();
  await expect(list).not.toBeVisible();
  await expect(page).toHaveURL(/#reuse-before-changing$/);
});
...
for (const locale of ["zh", "en"]) {
  for (const width of [375, 558, 733, 1024]) {
    test(`${locale} chapter menu geometry and navigation at ${width}px`, async ({ page }, testInfo) => {
      ...
      await expect(trigger).toHaveAttribute("data-value", "choose-your-path");
      await trigger.click();
      ...
      await list.getByRole("option").nth(3).click();
      await expect(list).not.toBeVisible();
      await expect(page).toHaveURL(/#reuse-before-changing$/);
      await expect.poll(async () => Math.round((await page.locator("#reuse-before-changing").boundingBox())!.y)).toBe(113);
      await expect(trigger).toHaveAttribute("data-value", "reuse-before-changing");
      ...
      await expect(list.getByRole("option", { selected: true })).toHaveAttribute("data-value", "reuse-before-changing");
      await expect.poll(async () => Math.round((await page.locator("#reuse-before-changing").boundingBox())!.y)).toBe(113);
...
test("chapter menu supports keyboard navigation, typeahead, Escape, and outside dismissal", async ({ page }) => {
  ...
  await expect(list.locator("[data-highlighted]")).toHaveAttribute("data-value", "your-first-page-checklist");
  await expect(list.locator("[data-highlighted]")).toHaveCSS("outline-width", "2px");
  await page.keyboard.press("End");
  await expect(list.locator("[data-highlighted]")).toHaveAttribute("data-value", "next-step");
  await page.keyboard.press("Home");
  await expect(list.locator("[data-highlighted]")).toHaveAttribute("data-value", "choose-your-path");
  await page.keyboard.type("Reuse");
  await expect(list.locator("[data-highlighted]")).toHaveAttribute("data-value", "reuse-before-changing");
  ...
  await trigger.click();
  await expect(list).toBeVisible();
  await page.getByRole("heading", { name: "Reuse before changing", exact: true }).click();
  await expect(list).not.toBeVisible();
  await trigger.focus();
  await trigger.press("Space");
  await expect(list).toBeVisible();
