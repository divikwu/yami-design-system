...
6. ProductSelection 配置以 `<id>@<version>` 共享；当前相关性流程先把每个已接受场景冻结为 4–16 件候选商品，并在多个来源分类之间轮转取样，避免高排名单一分类挤掉其他场景步骤。Page Merchandising Agent 再在冻结场景与商品池内正式选择 2–6 个可见场景（通常 3–5 个）及每场景 4–16 件商品：聚焦任务用 4 件，标准多步骤流程用 6–10 件，只有证据充分且新增商品能提供不同角色或有效选择时才使用 12–16 件，不为达到数量而填充近似重复款。TypeScript 校验来源、顺序、数量和去重后，同一正式结果同时回写结构预览与 PagePlan。首次提案若违反合同，运行时只携带确定性错误进行一次受限重试；第二次仍失败才进入显式 fallback。分类角色的 PageMerchandising `@2` 模板则完整继承 ready ProductSelectionResult 的模块商品、顺序、场景分组与全局去重，只补充展示语义以及 Hero/Shortcuts 引用，不重新选品或推断分类角色。
...
`relevance/default@1`、`relevance/intent-themes@2`、`relevance/intent-themes@3` 和
`relevance/intent-themes@4` 都不依赖 category-role 的 taxonomy 配置。`default@1` 保留固定排序
的旧任务回放，`intent-themes@2` 保留按目录分类直接成组的历史回放，`intent-themes@3` 保留
每场景 4–8 件的旧合同；当前默认的 `intent-themes@4` 会先用首轮 YAMI 聚合解析 ThemeIntent，
再接受可选的 Agent 分类与场景语义提案。商品归属、顺序、去重和每组 4–16 件的约束仍由
确定性模块校验；提案不足时回退到已验证的目录分类。
...
`category-role/landing-page-agent@1`；当前默认的 `relevance/intent-themes@4` 优先把已接受的
ThemeIntent 分类提案完整编译为 Shortcuts 与底部综合推荐的同源分类 Tab，不设置固定展示
数量上限；场景提案编译为 2–6 个 StartHere 候选主题，每个主题冻结 4–16 件商品并保留
`shoppingGoal`、理由和目录分类证据。没有足够提案时使用已验证的 YAMI 分类并明确标为规则
降级。随后由 Page Merchandising Agent 在冻结候选中正式复核可见场景和商品组合，再分别路由到
`topic-landing/brand-relevance@2`、`topic-landing/topic-relevance@2` 与
`topic-landing/campaign-relevance@2`。这些模板会用冻结的相关性商品生成 Hero、快捷入口、
