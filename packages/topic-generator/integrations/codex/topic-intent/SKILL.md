...
3. Read `intent`, `evidence.attempts`, `proposalReview`, and `confidence`. Do not create a proposal for an exact catalog brand or category.
4. If the baseline is ambiguous or low confidence and the phrase expresses a compound shopping scenario, prepare a `semantic-proposal/v1` JSON. Use only the user's wording and labels visible in the returned catalog evidence, then rerun:
...
7. Present the conclusion first, then the reason, Adapter attempts, proposal review, and evidence. Mark low-confidence or fallback results as requiring review.
...
- the concise, reviewable reason and confidence.
