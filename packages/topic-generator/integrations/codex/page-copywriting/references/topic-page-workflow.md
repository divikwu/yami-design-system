# Topic Page workflow for Page Copywriting

Use this workflow only when the caller supplies a Topic Generator
`content-writing` stage, a ready `TopicPagePlan`, `contentTaskId` tasks, or asks
for a `TopicPageContentProposal` / `TopicPageContentSpec`.
...
then read [the content proposal contract](topic-page-content-contract.md). Process only
